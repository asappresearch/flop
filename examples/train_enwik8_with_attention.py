...
class Custom_U(nn.Module):
    def __init__(self, in_features, out_features,
        ...
        self.set_mask(mem_length)

#    def set_mask(self, mem_length, same_length=False):
#        ones = self.linear1.weight.new_ones(mem_length, mem_length * 2)
#        if same_length:
#            self.attn_mask = nn.Parameter(
#                (torch.triu(ones, diagonal=mem_length+1) +
#                 torch.tril(ones, diagonal=-1)) * -10000.0,
#                requires_grad=False
#            )
#        else:
#            self.attn_mask = nn.Parameter(
#                torch.triu(ones, diagonal=mem_length+1) * -10000.0,
#                requires_grad=False
#            )

    def set_mask(self, mem_length, same_length=False):
        ones = self.linear1.weight.new_ones(mem_length * 2, mem_length * 2)
        if same_length:
            self.attn_mask = nn.Parameter(
            ...
            self.attn_mask = nn.Parameter(
                torch.triu(ones, diagonal=1) * -10000.0,
                requires_grad=False
            )
    ...
    def forward(self, input, attn_mask=None, memory=None):
        ...
        if memory is not None:
            assert list(memory.size()[-2:]) == [bsz, in_dim]
            mem_len = memory.size(0)
            src_len = input.size(0) + memory.size(0)
        ...
        q = q.contiguous().view(tgt_len, -1, proj_dim).transpose(0, 1)
        k = k.contiguous().view(src_len, -1, proj_dim).transpose(0, 1)
        v = v.contiguous().view(src_len, -1, proj_dim).transpose(0, 1)
...
class Model(nn.Module):
    ...
    def set_mask(self, mem_length, same_length=False):
        for r in self.rnn.rnn_lst:
            if isinstance(r.custom_u, Custom_U):
                r.custom_u.set_mask(mem_length, same_length=same_length)

    def forward(self, x, y, hidden, *memory):
        emb = self.drop(self.embedding_layer(x))
        if memory is not None:
            #output, hidden, memory = self.rnn(emb, hidden, memory=memory)
            ret = self.rnn(emb, hidden, None, *memory)
...
def eval_model(model, valid):
    with torch.no_grad():
        ...
        memory = [None] * (args.depth + 1)
        N = (len(valid[0])-1)//unroll_size + 1
        for i in range(N):
            x = valid[0][i*unroll_size:(i+1)*unroll_size].cuda()
            y = valid[1][i*unroll_size:(i+1)*unroll_size].cuda()
            hidden.detach_()
            #output, hidden, memory = model(x, hidden, memory=memory)
            ret = model(x, y, hidden, *memory)
            loss, hidden, memory = ret[0], ret[1], ret[2:]
...
def main(args):
    ...
    dev = create_batches(dev, eval_batch_size)
    test = create_batches(test, eval_batch_size)
    ...
    for epoch in range(args.max_epoch):
        ...
        for i in range(N):
            x = train[0][i*unroll_size:(i+1)*unroll_size].cuda(1)
            y = train[1][i*unroll_size:(i+1)*unroll_size].cuda(1)
            hidden.detach_()
            for m in memory:
                if m is not None:
                    m.detach_()
            ...
            #output, hidden, memory = model(x, hidden, memory=memory)
            ret  = para_model(x, y, hidden, *memory)
            loss, hidden, memory = ret[0], ret[1], ret[2:]
            loss = loss.mean()
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--eval_batch_size", type=int, default=0)
