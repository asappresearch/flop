...
dev_writer = SummaryWriter(os.path.join(args.work_dir, "dev"))
...
if args.prune:
    ...
    lambda_2 = nn.Parameter(torch.tensor(0.).to(device))
    optimizer_max = optim.Adam(
        [lambda_1, lambda_2],
        lr = args.prune_lr,
        weight_decay = 0
    )
    optimizer_max.param_groups[0]['lr'] = -args.prune_lr
    logging('Instantiated optimizer for lambda')
...
if args.prune:
    ...
    def lr_lambda(step):
        # return a multiplier instead of a learning rate
        if step == 0 and args.warmup_step == 0:
            return 1. / (args.d_model ** 0.5)
        else:
            return 1. / (args.d_model ** 0.5) / (step ** 0.5) if step > args.warmup_step \
                   else step / (args.d_model ** 0.5) / (args.warmup_step ** 1.5)

    scheduler_max = optim.lr_scheduler.LambdaLR(optimizer_max, lr_lambda=lr_lambda)
    logging('Instantiated scheduler for lambda lr')
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        if train_step % args.log_interval == 0:
            ...
            if args.prune:
                train_writer.add_scalar("loss/lagrangian_loss", lagrangian_loss, train_step)
                train_writer.add_scalar("lambda/1", lambda_1, train_step)
                train_writer.add_scalar("lambda/2", lambda_2, train_step)
                train_writer.add_scalar("sparsity/expected_sparsity", expected_sparsity, train_step)
                train_writer.add_scalar("sparsity/target_sparsity", target_sparsity, train_step)
