...
parser.add_argument("--max_step", type=int, default=300000, help="upper epoch limit")
parser.add_argument("--batch_size", type=int, default=32, help="batch size")
parser.add_argument(
    "--batch_chunk", type=int, default=1, help="split batch into chunks to save memory"
)
parser.add_argument(
    "--tgt_len", type=int, default=512, help="number of tokens to predict"
)
parser.add_argument(
    "--eval_tgt_len",
    type=int,
...
parser.add_argument(
    "--mem_len", type=int, default=512, help="length of the retained previous heads"
)
parser.add_argument(
    "--not_tied",
    action="store_true",
...
parser.add_argument(
    "--eval-interval", type=int, default=8000, help="evaluation interval"
)
parser.add_argument(
...
parser.add_argument("--gpu0_bsz", type=int, default=8, help="batch size on gpu 0")
...
parser.add_argument("--prune_sparsity", type=float, default=0.8)
parser.add_argument("--prune_end_epoch", type=int, default=10)
parser.add_argument("--l1_lambda", type=float, default=0.00001)
...
logging = create_exp_dir(
    args.work_dir, scripts_to_save=[], debug=args.debug
)
...
def train(epoch):
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        if args.prune:
            pruner.begin_iter(epoch, train_step, N)
        ...
        if args.prune:
            pruner.end_iter(epoch, train_step, N)
        ...
        if train_step % args.eval_interval == 0:
            ...
            if args.prune:
                ...
                dev_writer.add_scalar(
                    "model_size/current_prunable", pruned_size, train_step
                )
...
try:
    for epoch in itertools.count(start=0):
        train(epoch)
