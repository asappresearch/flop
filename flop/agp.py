...
class NervanaPruner(object):
    ...
    def __init__(self, model: nn.Module, subpruners: Dict[str, Dict[str, Any]]):
        ...
        for name, kwargs in subpruners.items():
            ...
            for key, value in kwargs.items():
                ...
                if key in {'starting_epoch', 'ending_epoch', 'epochs'}:
                    raise ValueError('Please provide arguments to Flambe by step (e.g. `starting_step`, `ending_step`, `steps`) instead of by epoch (e.g. `starting_epoch`, `ending_epoch`, `epochs`).')
        ...
        self.compression_scheduler = dict_config(model, None, {'pruners': pruners, 'policies': policies})
            ...
            for policy in policy_lst:
                for name in policy.pruner.params_names:
                    if name not in model_param_names:
                        raise ValueError(f'Weight `{name}` was marked for pruning at step {step}, but does not exist in model!')
    ...
    def begin_iter(self, step: int, n_iter: int, iter_per_step: int):
        self.compression_scheduler.on_minibatch_begin(step, minibatch_id=n_iter, minibatches_per_epoch=iter_per_step)

    def end_iter(self, step: int, n_iter: int, iter_per_step: int):
        self.compression_scheduler.on_minibatch_end(step, minibatch_id=n_iter, minibatches_per_epoch=iter_per_step)
